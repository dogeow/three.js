// {title: "编译器寄存器分配 — Chaitin-Briggs 图着色可视化"}
...
function updateUI(phase, message, registerMap) {
  ...
  if (registerMap) {
    ...
    if (entries.length === 0) regLines = '  (尚未分配)';
  ...
  uiEl.innerHTML = `
    ...
    ${regLines ? `<div style="margin-top:6px;color:#aaaaaa;">寄存器:${regLines}</div>` : ''}${spillStr}
  `;
}
...
async function simplifyStep() {
  ...
  phase = 'SIMPLIFY';
  highlight(node);
  updateUI(phase, `将 "${node}" (度=${currentDegree[node]} < ${K}) 压入栈中`, colored);
  ...
  markStacked(node);
  updateUI(phase, `"${node}" 已从图中移除`, colored);
...
async function spillStep() {
  ...
  phase = 'SPILL';
  highlight(spillNode);
  updateUI(phase, `⛔ 溢出 "${spillNode}" (度=${currentDegree[spillNode]} ≥ ${K}): 存储至内存`, colored);

  await sleep(stepDelay);

  colored[spillNode] = 'spill';
  markSpilled(spillNode);
  updateUI(phase, `"${spillNode}" 已标记为溢出`, colored);
...
async function selectPhase() {
  phase = 'SELECT';
  updateUI(phase, '从栈中弹出并分配寄存器…', colored);
  ...
  while (simplifyStack.length > 0) {
    ...
    highlight(node);
    phase = `SELECT: ${node}`;
    updateUI(phase, `弹出 "${node}" — 寻找安全颜色`, colored);
  ...
  const spilledMsg = spilled.length
    ? `\n  ⛔ 溢出变量: ${spilled.join(', ')}\n  ( 溢出变量需要从内存重新加载，产生额外开销 )`
    : '\n  ✅ 所有变量已分配寄存器 — 无溢出!';
  updateUI(phase, `着色完成!${spilledMsg}`, colored);
}
...
async function runColoring() {
  ...
  updateUI('就绪', '开始 Chaitin-Briggs 着色算法…', colored);
...
btn.addEventListener('click', () => {
  if (!animating) { runColoring(); btn.textContent = '⏸ 运行中…'; }
  else { animating = false; btn.textContent = '▶ 运行着色'; }
});
...
`;
legend.innerHTML = `
  <div style="margin-bottom:6px;color:#00e5ff;font-weight:bold;">图例</div>
...
// Start hint
updateUI('就绪', '点击 ▶ 运行着色 开始 Chaitin-Briggs 算法', {});
