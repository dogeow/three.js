...
// Ground
const ground = new THREE.Mesh(new THREE.PlaneGeometry(200, 200), new THREE.MeshStandardMaterial({ color: 0x2a3040, roughness: 0.9 }))
ground.rotation.x = -Math.PI / 2
ground.receiveShadow = true
scene.add(ground)

// Grid
const grid = new THREE.GridHelper(200, 40, 0x4a5a6f, 0x2a3446)
grid.position.y = 0.02
scene.add(grid)

// Road network
const ROAD_COLOR = 0x3a4050
...
class Car {
  constructor() {
    ...
    const bodyMat = new THREE.MeshStandardMaterial({ color, roughness: 0.3, metalness: 0.4, emissive: new THREE.Color(color).multiplyScalar(0.1), emissiveIntensity: 0.5 })
    ...
    const lightMat = new THREE.MeshStandardMaterial({ color: 0xffffcc, emissive: 0xffffcc, emissiveIntensity: 1.0 })
...
hud.style.cssText = 'position:fixed;top:16px;left:16px;color:#ffffff;font-family:monospace;font-size:13px;pointer-events:none;line-height:1.8;text-shadow:0 1px 3px #000'
