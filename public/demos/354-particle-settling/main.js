...
scene.fog = new THREE.FogExp2(0x1a2438, 0.006);
const camera = new THREE.PerspectiveCamera(60,innerWidth/innerHeight,0.1,500);
camera.position.set(0,40,80);
const controls = new OrbitControls(camera,renderer.domElement);
controls.enableDamping=true;
scene.add(new THREE.AmbientLight(0xbfd4ff,0.9));
scene.add(new THREE.HemisphereLight(0xffe8c2, 0x2b3550, 0.9));
const sun = new THREE.DirectionalLight(0xfff1cc, 3.2);
sun.position.set(40, 80, 30);
scene.add(sun);
const sunFill = new THREE.DirectionalLight(0xbfd4ff, 1.2);
sunFill.position.set(-30, 40, -20);
scene.add(sunFill);
const pl = new THREE.PointLight(0xff8a3d,2.2,120); pl.position.y=20; scene.add(pl);
renderer.toneMapping = THREE.ACESFilmicToneMapping;
renderer.toneMappingExposure = 1.2;
const floor = new THREE.Mesh(new THREE.PlaneGeometry(80,80),new THREE.MeshStandardMaterial({color:0x3b4466,roughness:0.9}));
