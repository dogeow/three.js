...
const LEAF_NAMES = [
  ...
  'Macaca mulatta', 'Papio anubis', 'Mus musculus', 'Rattus norvegicus',
  'Canis lupus', 'Felis catus', 'Bos taurus', 'Sus scrofa',
  'Gallus gallus', 'Danio rerio', 'Drosophila', 'Caenorhabditis',
  'Arabidopsis', 'Zea mays', 'Saccharomyces', 'Escherichia',
]
...
function generateTree(depth) {
  ...
  while (currentLevel.length > 2 && currentDepth > 1) {
    ...
    for (let i = 0; i < currentLevel.length; i += 2) {
      ...
      const parentName = `Clade ${currentDepth}_${idCounter}`
  ...
  // Root node
  const root = new TreeNode(idCounter++, 'Root', false, 0)
