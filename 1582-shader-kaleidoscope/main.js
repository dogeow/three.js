...
const mat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `uniform float uTime; uniform vec2 uRes; varying vec2 vUv;
    ...
    void main() {
      vec2 uv = (vUv - 0.5) * 2.0;
      uv = kaleido(uv, 6.0 + sin(uTime * 0.5) * 2.0);
      vec3 col = vec3(0.0);
      for (int i = 0; i < 3; i++) { float t = uTime * 0.5 + float(i) * 0.3; vec2 p = uv + vec2(sin(t + float(i)), cos(t * 1.3 + float(i))) * 0.5; col[i] += 0.5 + 0.5 * sin(length(p) * 10.0 - t * 3.0); }
